...
class Server:
    ...
    def __init__(self, workspace: Workspace, is_in_debug_mode: bool = False) -> None:
        ...
        self.app.add_url_rule(
            '/api/projects/<int:project_id>/attributions',
            'get_attributions',
    ...
    def run(self, host: str = 'localhost', port: int = 8000) -> None:
        ...
        Args:
            ...
            port (int): The port at which the application should run. Defaults to 8000.
        """
    ...
    def get_attributions(self, project_id: int) -> flask.Response:
        """Retrieves all attributions specified in the "indices" URL parameter, which is a comma-separated list of attribution indices.

        Args:
            project_id (int): The ID of the project from which the attribution is to be retrieved.

        Returns:
            flask.Response: Returns an HTTP 200 OK response with a JSON string as content, which contains an array with the data of the attributions.
                If the specified project does not exist, then an HTTP 404 Not Found response is returned. If any of the attributions specified in the
                "indices" URL parameter does not exist, then an HTTP 404 Not Found response is returned.
        """
        ...
        # Parses the "indices" URL parameter, which is a comma-separated list of attribution indices, and retrieves the attributions with the
        # specified indices from the project
        attributions_json: list[AttributionJson] = []
        image_mode = flask.request.args.get('imageMode', 'input')
        for attribution_index in [int(index) for index in flask.request.args.get('indices', '').split(',')]:
            ...
            # Adds the attribution to the list of attributions
            attributions_json.append(attribution_json)

        # Returns the retrieved attributions
        return http_ok(attributions_json)
    ...
    def get_color_map_preview(self, color_map: str) -> flask.Response:
        ...
        heatmap = numpy.linspace(-1.0, 1.0, dtype=numpy.float64, num=width)
